...
void print_allocated_memory()
{
  ...
  while(del) {
    ...
    printf("%d ALLOCATED MEMORY AT %p %d bytes (%d bytes REMAINING)\n",
...
{
  printf("start memleak tracking.\n");
  mem_trace_en = 1;
}

void disable_memleak_trace(int signum __unused)
{
  printf("stop memleak tracking.\n");
  mem_trace_en = 0;
}
...
/*  minimum = atoi(prop);*/
  ...
  if(enable_memleak){
    ...
    __real_free = __free;
    printf("memleak tracer attached");
  } else {
  #endif
    ...
    __real_free = free;
    printf("memleak tracer not attached");
  #ifdef MEMLEAK_FLAG
  }
  #endif
}
...
void * server_memleak_thread (void * param __unused)
{
  ...
  printf("server_memleak_thread started");
  ...
  while(redo)
  {
    ...
    switch(server_memleak_event)
    {
      ...
      case MEMLEAK_EXIT:
        ...
        break;
      default:
        printf("Something bad, should never be printed");
        pthread_mutex_unlock (&server_memleak_mut);
     }
  }
  printf("server_memleak_thread exit");
  pthread_exit(0);
}
