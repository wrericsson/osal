...
static inline void add(int fd_value)
{
  ...
  fdlist->bt_depth = function_stacktrace(fdlist->bt, MAX_BACKTRACE_DEPTH);
  fdlist->next = head;
  head = fdlist;
  fdleak_count++;
  pthread_mutex_unlock(&osal_debug_fdleak_mut);
}

void fdleak_dump_list()
{
...
int __open(const char* dev_name, int flags, ...)
{
  ...
  if (errno == EMFILE) {
    printf( "FATAL during open %s Restart camera daemon !!!",strerror(errno));
    raise(SIGABRT);
    raise(SIGKILL);
  }
    if (fd_value > 0) {
...
int __pipe(int fd[])
{
  ...
  ret_value = pipe(fd);
  if (errno == EMFILE) {
    printf( "FATAL during pipe creation %s Restart camera daemon !!!",strerror(errno));
    raise(SIGABRT);
    raise(SIGKILL);
  }
  if (ret_value >= 0) {
...
int __socket(int domain, int type, int protocol)
{
  ...
  if (errno == EMFILE) {
    printf( "FATAL during socket create %s Restart camera daemon !!!",strerror(errno));
    raise(SIGABRT);
    raise(SIGKILL);
  }
  if (ds_fd > 0) {
...
void* __mmap(void* addr, size_t size, int prot, int flags, int fd, off_t offset)
{
  ...
  if (errno == EMFILE) {
    printf( "FATAL during mmap %s Restart camera daemon !!!",strerror(errno));
    raise(SIGABRT);
    raise(SIGKILL);
  }
  if (fd > 0) {
...
int __wrap_open(const char* dev_name, int flags, ...)
{
  mode_t mode = 0;
...
void*  __wrap_mmap(void* addr, size_t size, int prot, int flags, int fd, off_t offset)
{
  return __real_mmap(addr, size, prot, flags, fd, offset);
}

int  __wrap_close(int fd_value)
{
...
void osal_degug_dump_fdleak_trace()
{
    ...
    __real_close = close;
    if (fdleak_count) {
      printf("FATAL fdleak found in camera daemon %d",
         fdleak_count);
      fdleak_dump_list();
    }
}
