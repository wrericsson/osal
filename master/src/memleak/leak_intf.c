...
void dump_list_of_daemon_fd()
{
  ...
  dir = opendir(path);
  if (dir == NULL) {
    printf("Failure in opening %s", path);
    return;
  }
  ...
  printf("WARNING! Number of active devices exceeds %d. \
  ...
  printf("Restarting camera daemon!");
  exit(0);
}
...
void* server_debug_dump_data_for_sof_freeze(void *status)
{
  ...
  if (debug_file_fd < 0) {
    debug_file_fd = open (filename, O_RDWR);
    if (debug_file_fd < 0) {
      printf("Failure in opening %s", filename);
      return NULL;
    }
  }
  ...
  while ((de = readdir(dir))) {
    ...
    dir1 = opendir(input);
    if (dir1 == NULL) {
      printf("opendir of %s fails", input);
      close(debug_file_fd);
      closedir(dir);
      return NULL;
    }
  ...
  if (dir == NULL) {
    printf("opendir regulator fails");
    close(debug_file_fd);
    return NULL;
  }
  while ((de = readdir(dir))) {
